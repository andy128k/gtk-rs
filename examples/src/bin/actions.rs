...
impl ExampleApplication {
    ...
    fn activate(&self) {
        if let Some(window) = self.0.get_active_window() {
            window.show_all();
        } else {
...
impl ExampleApplication {
    fn about(&self) {
        let window = self.0.get_active_window();
    ...
    fn quit(&self) {
        // Close all windows
        for window in self.0.get_windows() {
            window.close();
        }
    }
}
...
impl ExampleWindow {
    pub fn new(app: &gtk::Application) -> Self {
        ...
        volume.connect_change_value(
            glib::clone!(@weak window, @weak volume_action => move |_scale, _scroll_type, value| {
        ...
        volume_action.connect_property_state_notify(
            glib::clone!(@weak volume_label => move |action| {
                let value = action.get_state().and_then(|variant| variant.get::<f64>()).unwrap();
...
impl ExampleWindow {
    ...
    #[action(name = "volume")]
    fn volume_activate(&self, _state: f64, value: f64) -> Option<f64> {
        Some(value)
    }
...
fn main() {
    ...
    application.0.run(&args().collect::<Vec<_>>());
}
