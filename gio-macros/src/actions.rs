...
impl ChangeStateHandler {
    fn try_from_signature(sig: Signature) -> Result<Self, Error> {
        if !is_assoc(&sig) {
            return Err(Error::new(
                sig.span(),
...
impl ActionInfo {
    fn try_from_handlers(name: String, handlers: Vec<ActionHandlerInfo>) -> Result<Self, Error> {
        ...
        let change_state_handler = change_state_handler
            .map(|h| ChangeStateHandler::try_from_signature(h.sig))
            .transpose()?;
    ...
    fn parameter_type(&self) -> Result<Option<&Type>, Error> {
        if let Some(ref h) = self.activate_handler {
            h.parameter_type()
        } else {
            Ok(None)
        }
...
fn get_state(info: &ActionInfo, arg: &FnArg) -> Result<TokenStream2, Error> {
    ...
    Ok(quote_spanned! { arg.span() =>
        match action.get_state().and_then(|variant| variant.get::<#state_type>()) {
            Some(value) => value,
            None => {
                glib::g_critical!("actions", "Action {} expects a state of type {} but has `{:?}`.", #action_name, stringify!(#state_type), action.get_state());
                return;
            }
        }
    })
...
fn generate_activate_handler(
    ...
    let handler = quote_spanned! { handler.sig.span() =>
        action.connect_activate(
            glib::clone!(@weak self as this => move |action, parameter| {
                #invoke
            })
        );
    };
